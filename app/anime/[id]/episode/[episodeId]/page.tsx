...
export async function generateMetadata({ params }: WatchPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      images: episode.season.anime.imageUrl ? [episode.season.anime.imageUrl] : [],
      type: "video.episode",
    },
    twitter: {
...
export default async function WatchPage({ params }: WatchPageProps) {
  ...
  if (episode.season.anime.id !== id) {
    console.warn(`Mismatch between URL anime ID (${id}) and episode anime ID (${episode.season.anime.id})`);
      ...
      <nav className="sticky top-0 z-50 border-b border-zinc-200 bg-white/80 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-900/80">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          >
            ...
            <span className="hidden sm:inline">Voltar para {episode.season.anime.title}</span>
            ...
            <div className="mt-6 flex flex-col gap-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  {prevEpisode && (
                    <Link
                      href={`/anime/${id}/episode/${prevEpisode.id}`}
                  ...
                  {nextEpisode && (
                    <Link
                      href={`/anime/${id}/episode/${nextEpisode.id}`}
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-bold">{episode.season.anime.title}</h2>
                ...
                <div className="mt-6 border-t border-zinc-100 pt-6 dark:border-zinc-800">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-400">Sinopse do Anime</h3>
                  <p className="mt-2 text-zinc-600 dark:text-zinc-400 leading-relaxed">
                    {episode.season.anime.description || "Sem descrição disponível."}
                  </p>
                </div>
              </div>
            </div>
