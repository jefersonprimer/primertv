...
import {
  ...
  ResetPasswordState,
} from "@/app/actions/resetPassword";
...
export default function ForgotPasswordPage() {
  ...
  // Request Code Action
  const [requestCodeState, requestCodeAction, isRequestPending] =
    useActionState(
  ...
  // Verify Code Action
  const [verifyCodeState, verifyCodeAction, isVerifyPending] = useActionState(
    async (prevState: ResetPasswordState | undefined, formData: FormData) => {
      const result = await verifyPasswordResetCode(prevState, formData);
      if (result.success) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[80vh] md:h-auto md:min-h-[calc(100vh-4rem)] bg-black text-white px-4 py-8">
      <div className="w-full h-full md:h-auto md:max-w-md p-6 sm:p-8 space-y-6 flex flex-col justify-center">
        {step === 1 && (
          <>
            ...
            <form action={requestCodeAction} className="space-y-5">
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    defaultValue={email}
                    className="w-full p-2 py-2.5 border border-zinc-700 rounded-lg bg-zinc-800/80 text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-500 disabled:opacity-50 transition-colors shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2"
              >
                {isRequestPending
                  ? t("sendingCodeButton")
                  : t("sendCodeButton")}
          ...
          <>
            <div className="text-center space-y-2">
              <h2 className="text-2xl font-bold">
                {isCodeVerified ? t("forgotPasswordTitle") : t("enterCodeTitle")}
              </h2>
              <p className="text-sm text-zinc-400">
              ...
              <>
                <form action={verifyCodeAction} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-500 disabled:opacity-50 transition-colors shadow-lg shadow-blue-600/20"
                  >
                    {isVerifyPending
                      ? t("verifyingCodeButton")
                      : t("verifyCodeButton")}
                  </button>
                </form>
