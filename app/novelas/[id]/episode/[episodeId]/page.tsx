...
export async function generateMetadata({ params }: WatchPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      images: episode.season.novela.imageUrl ? [episode.season.novela.imageUrl] : [],
      type: "video.episode",
    },
    twitter: {
      ...
      <nav className="sticky top-0 z-50 border-b border-zinc-200 bg-white/80 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-900/80">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          >
            ...
            <span className="hidden sm:inline">Voltar para {episode.season.novela.title}</span>
            ...
            <div className="mt-6 flex flex-col gap-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  {prevEpisode && (
                    <Link
                      href={`/novelas/${id}/episode/${prevEpisode.id}`}
                  ...
                  {nextEpisode && (
                    <Link
                      href={`/novelas/${id}/episode/${nextEpisode.id}`}
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-bold">{episode.season.novela.title}</h2>
                    <p className="mt-1 text-zinc-500">
                      Temporada {episode.season.number} • Capítulo {episode.number}
                    </p>
                  </div>
                </div>
                <div className="mt-6 border-t border-zinc-100 pt-6 dark:border-zinc-800">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-400">Sinopse da Novela</h3>
                  <p className="mt-2 text-zinc-600 dark:text-zinc-400 leading-relaxed">
                    {episode.season.novela.description || "Sem descrição disponível."}
                  </p>
                </div>
              </div>
            </div>
