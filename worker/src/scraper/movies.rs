    ...
    pub async fn scrape(&self, title: &str) -> Result<Option<ScrapedMedia>> {
        for base_url in &self.base_urls {
            ...
            match try_scrape_from_site(&self.client, base_url, title).await {
                Ok(Some(media)) => {
                    println!("Successfully scraped movie {} from {}", title, base_url);
                    return Ok(Some(media));
                }
                Ok(None) => {
                    println!("No results for movie {} on {}", title, base_url);
                    continue;
                }
                Err(e) => {
                    eprintln!("Error scraping movie from {}: {}", base_url, e);
                    continue;
                }
            }
        }
